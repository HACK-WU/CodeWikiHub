...
## 简介
本文件面向"记忆系统"的完整配置与运维，覆盖自动沉淀机制、查询优化策略、归档与冷热分离、代码片段记忆的提取与禁忌规则，以及可落地的配置文件示例与最佳实践。尽管部分配置文件已被移除，但核心的记忆持久化、查询优化和归档策略功能仍然通过以下机制得到支持：
- KB 通道：本地索引与缓存（group-index、relations-cache、local KB），用于快速命中与原文定位。
- 向量通道：通过 zvec 引擎进行语义检索，支持混合检索、重排与标签过滤。
...
**章节来源**
- [docs/configuration.md:130-163](file://docs/configuration.md#L130-163)
...
**章节来源**
- [docs/configuration.md:67-88](file://docs/configuration.md#L67-88)
