...
## 更新摘要
**变更内容**
- 移除了对已删除的 `.claude/skills/gitnexus/*` 技能文件的引用
- 更新了项目结构部分，反映当前实际存在的技能目录
- 简化了依赖分析，聚焦于现有 ki-search 技能和相关文档
- 保持了核心概念和使用模式的完整性
...
## 引言
本规范面向Agent技能（SKILL）的创建、组织、调用与发布，目标是让团队以统一的结构和流程开发可复用、可测试、可发布的技能。文档基于仓库中现有的技能样例与工作流说明，提炼出：
- SKILL.md 的结构与元数据约定
- 行为规则描述与工具调用规范
- 从需求到发布的完整流程
- 技能间依赖关系与冲突处理
...
- skills/agents-md-init/AGENTS-template.md：AGENTS.md 的标准格式模板
- docs/ 下提供工作流、索引管理与验证等配套文档

```mermaid
graph TB
A["根级入口<br/>AGENTS.md / CLAUDE.md"] --> B["知识库检索技能<br/>skills/ki-search/SKILL.md"]
A --> C["AGENTS模板<br/>skills/agents-md-init/AGENTS-template.md"]
...
- [workflows.md:1-164](file://docs/workflows.md#L1-L164)
...
- AGENTS模板：提供标准化的AGENTS.md文件格式，包含项目记忆索引、用户画像、近期工作等章节结构。
...
### 组件B：AGENTS模板（项目记忆管理）
- 标准格式：包含项目记忆索引、用户画像、近期工作、更新日志等章节
- 记忆索引：从 ki 的 ${scope}-memory scope 获取真实数据填充
- 用户画像：直接存储于文件中，不写入 ki 记忆
- 近期工作：最近7天内的工作记录，超过时间移入归档
...
**图表来源**
- [AGENTS-template.md:14-118](file://skills/agents-md-init/AGENTS-template.md#L14-L118)
...
- [verify-index.md:1-214](file://docs/verify-index.md#L1-L214)
...
- 技能间依赖
  ...
  - AGENTS模板依赖 ki-search 技能进行项目记忆索引管理
...
**章节来源**
- [verify-index.md:173-214](file://docs/verify-index.md#L173-L214)
- [workflows.md:143-149](file://docs/workflows.md#L143-L149)
