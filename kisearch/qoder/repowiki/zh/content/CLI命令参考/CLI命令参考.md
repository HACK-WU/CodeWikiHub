...
## 更新摘要
**变更内容**
- 更新了search命令的scope参数说明，现在支持逗号分隔的多scope参数（如-s monitor,alerting）
- 在strict模式下，未注册的scope会被跳过而不是导致失败，并在响应中提供skipped信息
- 添加了多scope检索的行为说明和示例
- 更新了相关错误处理和故障排查指南
...
- 关键参数
  ...
  - -s, --scope <scope>：项目隔离标识（**支持逗号分隔的多scope参数**，如 `-s monitor,alerting`）
  - --limit <n>：返回条数上限（默认 10）
  - --threshold <n>：相似度阈值（默认 0）
  - --tags <tags>：过滤标签（逗号分隔，OR）
  - --original：返回 local KB 文件级原文（默认不返回）
- 行为要点
  - **多scope聚合检索**：当传入多个scope时，单次查询跨scope召回，结果按得分统一排序，每条命中标注来源 `scope`
  - **strict模式下的容错处理**：在strict模式下，未注册的scope会被跳过而不是导致失败，并在响应的 `skipped` 字段中提供跳过原因
...
- 示例
  ...
  - **新增**：ki search "监控告警" -s monitor,alerting,logging  # 多scope聚合检索
  - **新增**：ki search "配置管理" -s team-a,ghost  # strict模式下ghost会被跳过
...
```json
{
  ...
  "skipped": [
    {
      "scope": "ghost",
      "reason": "未注册（scopeMode=strict，不在配置 scopes 白名单）"
    }
  ]
}
```
...
章节来源
- [src/search.ts:83-127](file://src/search.ts#L83-L127)
- [src/search.ts:291-322](file://src/search.ts#L291-L322)
- [test/search-multiscope.test.ts:80-122](file://test/search-multiscope.test.ts#L80-L122)
...
- **多scope优化**：search命令在多scope模式下使用单次查询+scope OR过滤，embedding仅执行1次，提升检索效率。
...
- **多scope检索问题**：
  - 在strict模式下，如果所有scope都未注册，会返回 `ok:false` 并提示"无可检索的 scope"
  - 部分scope未注册时，有效的scope会继续检索，未注册的会在 `skipped` 字段中列出
  - 检查响应中的 `skipped` 字段了解哪些scope被跳过及原因
- MCP 启动失败：
...
## 结论
ki CLI 提供了完整的知识库导入、索引管理、检索、存储与服务管理能力。通过统一的参数规范、健壮的参数校验与错误处理、以及完善的运维工具链，用户可以高效地完成从数据入库到检索应用的全流程。**最新的搜索命令支持多scope聚合检索**，在strict模式下具有更好的容错性，能够跳过未注册的scope继续检索其他有效scope，提升了使用的灵活性和鲁棒性。建议在生产环境中结合 doctor 预检、backup/restore 快照机制与 mcp 共享单例模式，保障稳定性与可维护性。
...
- 常用工作流最佳实践
  ...
  - **多scope检索**：使用逗号分隔的scope参数进行跨项目检索，如 `ki search "监控配置" -s monitor,alerting,logging`
- 性能优化建议
  - 合理设置 chunk-size/chunk-overlap 平衡检索精度与体积
  - 使用 tags 精确过滤缩小检索范围
  - 批量写入优先使用 bulk-store
  - 长驻服务使用 mcp --http 共享单例，避免多进程锁竞争
  - **多scope检索优化**：利用单次查询+scope OR过滤的特性，减少embedding调用次数
