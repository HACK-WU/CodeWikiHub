...
**最新更新**：实现了多标签 OR 查询的性能优化，将嵌入操作从 N 次减少到 1 次，并增强了搜索结果中的标签信息展示。
...
**章节来源**
- [src/search.ts:1-329](file://src/search.ts#L1-L329)
...
- [src/lib/relation-map.ts:1-121](file://src/lib/relation-map.ts#L1-L121)
...
**章节来源**
- [src/search.ts:20-52](file://src/search.ts#L20-L52)
...
### 标签过滤系统（优先级规则与匹配逻辑）
- **性能优化**：实现了多标签 OR 查询优化，将嵌入操作从 N 次减少到 1 次。当不显式传 tags 时，系统会先收集所有 scope 下的标签并集，然后执行单次查询，而不是对每个标签分别执行 embedding 操作。
...
**图表来源**
- [src/search.ts:136-188](file://src/search.ts#L136-L188)
...
### 查询性能优化技巧
- **多标签 OR 查询优化**：实现了显著的性能提升，将嵌入操作从 N 次减少到 1 次。当不显式传 tags 时，系统会先收集所有 scope 下的标签并集，然后执行单次查询，而不是对每个标签分别执行 embedding 操作。topk 按 tag 数放大保障每 tag 召回上限，查询后按 tag 分组限额 + TAG_PRIORITY 排序。
...
## 性能考虑
- **多标签 OR 查询性能优化**：通过单次查询替代逐标签分查，显著减少了嵌入操作的次数。对于包含 N 个标签的查询，性能提升可达 N 倍。
...
## 结论
该搜索模块通过"向量语义 + BM25 全文"的双路径召回与 RRF 融合排序，兼顾了语义理解与关键词匹配的鲁棒性；配合标签过滤、原文定位与多级去重，满足复杂知识检索需求。工程层面通过锁管理、批处理与缓存等手段保障高可用与高性能。**最新的性能优化通过多标签 OR 查询机制，将嵌入操作从 N 次减少到 1 次，显著提升了大规模标签场景下的查询性能。**
...
- [docs/cli.md:475-539](file://docs/cli.md#L475-L539)
- [bin/ki.mjs:75-113](file://bin/ki.mjs#L75-L113)
- [src/search.ts:285-318](file://src/search.ts#L285-L318)
- [src/lib/vector-client.ts:477-506](file://src/lib/vector-client.ts#L477-L506)