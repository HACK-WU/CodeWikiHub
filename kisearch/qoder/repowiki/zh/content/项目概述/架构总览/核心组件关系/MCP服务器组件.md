...
## 更新摘要
**变更内容**
- 新增请求源跟踪机制：基于AsyncLocalStorage的请求级上下文管理，支持MCP工具名和API端点路径的自动提取
- 增强撞锁日志可观测性：向量库锁冲突日志现包含精确的请求来源标识，便于问题定位
- 完善HTTP模式监控：/healthz接口新增鉴权失败计数，支持运维监控告警
...
此外，组件内置 Token 鉴权系统（多 Token + scope 授权）、守护进程管理、端口复用与健康检查等能力，并提供 11 个核心工具的统一注册与生命周期管理。**最新更新**：引入基于 AsyncLocalStorage 的请求源跟踪机制，实现工具调用和 API 请求的精确来源追踪，显著提升锁冲突问题的诊断效率。
...
- 向量客户端与请求源跟踪：src/lib/vector-client.ts
...
A --> D["vector-client.ts<br/>请求源跟踪/向量操作"]
A --> E["mcp-tools/*<br/>11 个工具注册"]
B --> F["mcp-http-api.ts<br/>/api/* 扩展接口"]
D --> G["业务逻辑实现<br/>query-group / search / store ..."]
```
...
- [src/lib/vector-client.ts:91-106](file://src/lib/vector-client.ts#L91-L106)
...
- HTTP 传输与守护（mcp-http.ts）
  ...
  - **新增**：请求源跟踪，通过 describeMcpSource 函数提取工具名和 API 端点路径。
...
- 扩展 API（mcp-http-api.ts）
  ...
  - **新增**：API 端点路径作为请求源标识，集成到向量操作上下文。
- 向量客户端（vector-client.ts）
  - **新增**：基于 AsyncLocalStorage 的请求源跟踪机制，通过 runWithVectorSource 包装所有向量操作。
  - 撞锁重试时附带请求来源信息，显著提升问题诊断效率。
- 工具注册（mcp-tools/*）
...
## 架构总览
下图展示从请求到工具执行的端到端流程，涵盖鉴权、作用域校验、会话管理与工具调用，**新增请求源跟踪环节**。
...
H->>H : 鉴权(Bearer)/作用域校验
H->>V : runWithVectorSource(请求源标注)
V->>T : 创建 StreamableHTTPServerTransport
T->>S : connect()
C->>S : tools/call(name, args)
S->>R : 路由到对应工具
R->>V : 执行业务函数(带超时+请求源)
V->>V : probeWithRetry(撞锁日志含来源)
B-->>R : 结果
R-->>C : {content|isError}
```
...
- [src/lib/vector-client.ts:113-126](file://src/lib/vector-client.ts#L113-L126)
- [src/mcp-server.ts:49-71](file://src/mcp-server.ts#L49-L71)
...
- 健康检查
  ...
  - **新增**：authFailures 字段统计启动以来的鉴权失败次数，支持运维监控。
...
- AsyncLocalStorage 上下文管理
  - 基于 Node.js AsyncLocalStorage 实现请求级上下文隔离，确保多线程环境下上下文正确传递。
  - vectorSourceStorage 存储当前异步链的请求来源标识。
- 请求源提取
  - MCP 请求：describeMcpSource 函数从 JSON-RPC 请求体中提取工具名称（tools/call.name）或方法名。
  - API 请求：直接使用 API 端点路径作为来源标识（如 /api/import/run）。
  - 来源截断：限制在 80 字符以内，防止超长字符串污染日志。
- 撞锁日志增强
  - probeWithRetry 函数在检测到向量库锁冲突时，自动附带请求来源信息。
  - 日志格式：`[kisearch][mcp:ki_search] 向量库被其他进程占用...` 或 `[kisearch][/api/import/run] 向量库被其他进程占用...`
- 集成点
  - HTTP 请求入口：handleMcpPost 和 handleApiRequest 分别包装 MCP 和 API 请求。
  - 向量操作：所有向量客户端调用都通过 runWithVectorSource 包裹，确保上下文传播。
...
H->>H : describeMcpSource(body)
H->>V : runWithVectorSource("mcp : ki_search", fn)
V->>V : vectorSourceStorage.run(source, fn)
V->>Z : probe(dbPath)
Z-->>V : locked=true
V->>V : getVectorOpSource()
V->>V : 输出带来源的撞锁日志
```

**图表来源**
- [src/lib/mcp-http.ts:156-172](file://src/lib/mcp-http.ts#L156-L172)
- [src/lib/vector-client.ts:91-126](file://src/lib/vector-client.ts#L91-L126)
...
- mcp-http.ts 依赖：
  ...
  - net-addr.ts（地址判定）
  - vector-client.ts（runWithVectorSource）
...
A --> D["mcp-tools/*"]
B --> E["mcp-http-api.ts"]
B --> F["vector-client.ts"]
B --> G["net-addr.ts"]
F --> H["node:async_hooks"]
F --> I["zvec-engine"]
```
...
- [src/mcp-server.ts:49-71](file://src/mcp-server.ts#L49-L71)
- [src/lib/mcp-http.ts:344-499](file://src/lib/mcp-http.ts#L344-L499)
- [src/lib/vector-client.ts:16-31](file://src/lib/vector-client.ts#L16-L31)
- [src/lib/mcp-http-api.ts:216-272](file://src/lib/mcp-http-api.ts#L216-L272)
...
- I/O 与缓存
  ...
  - 上传与导入受控目录与大小限制，防止滥用。
- **新增**：请求源跟踪开销
  - AsyncLocalStorage 操作为轻量级上下文切换，对性能影响极小。
  - 请求源提取仅在请求入口处执行，避免重复计算。
...
- 常见错误与定位
  ...
  - **新增**：向量库锁冲突：现在日志包含精确的请求来源，如 `[kisearch][mcp:ki_search] 向量库被其他进程占用...`
- 诊断命令
...
- **新增**：锁冲突排查
  - 查看 stderr 日志中的带来源标识的锁冲突消息。
  - 根据来源标识快速定位是哪个工具或 API 触发了锁冲突。
  - 结合 /healthz 的 authFailures 字段监控鉴权问题趋势。
...
## 结论
MCP 服务器组件通过统一的启动器、HTTP 传输层、Token 鉴权系统与工具注册机制，提供了稳定、可扩展的知识库访问能力。stdio 模式适合简单场景，HTTP 模式适合多 IDE/Agent 共享与生产部署。结合守护进程、端口复用与健康检查，形成完整的运维闭环。11 个核心工具覆盖查询、写入、搜索、存储、管理等关键能力，并通过 RBAC 与作用域控制保障安全。**最新增强的请求源跟踪机制显著提升了锁冲突问题的诊断效率，使运维人员能够快速定位问题根源。**
...
- **新增**：锁冲突排查示例
  ...
  # 查看 stderr 日志中的带来源标识的锁冲突消息
  tail -f ~/.ki/logs/stderr.log | grep "向量库被其他进程占用"
  
  # 根据来源标识快速定位问题工具或 API
  grep "mcp:ki_search" ~/.ki/logs/stderr.log
  grep "/api/import/run" ~/.ki/logs/stderr.log
  ```
