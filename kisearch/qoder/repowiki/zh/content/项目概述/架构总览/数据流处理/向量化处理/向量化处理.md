...
## 更新摘要
**变更内容**
- 增强向量重建功能：支持200条/批的分批处理、实时进度报告、确定性文档ID生成和全面统计收集
- 新增批量向量化模块：bulkVectorize函数实现高效批量处理，支持进度回调和错误聚合
- 改进重建流程：支持局部重建（--group/--tags）、内存ID回写、标签合并去重
- 优化错误处理：批间进度反馈、失败条目精确定位、统计信息完整收集
...
**最新更新**：增强了向量重建功能，支持分批处理（200条/批）、实时进度报告、确定性文档ID生成和全面统计收集，提升了大规模知识库重建的性能和可观测性。
...
B --> J["batch-vectorize.ts<br/>批量向量化"]
J --> K["rebuild-vector.ts<br/>向量重建"]
```
...
- [rebuild-vector.ts:1-520](file://src/lib/rebuild-vector.ts#L1-L520)
...
- 批量向量化（batch-vectorize.ts）：支持单条、串行批量和批量向量化三种模式，200条/批处理，进度回调和错误聚合。
- 向量重建（rebuild-vector.ts）：从已还原KB重建三类向量，支持局部重建、标签合并、memoryId回写和全面统计。
...
- [rebuild-vector.ts:329-520](file://src/lib/rebuild-vector.ts#L329-L520)
...
- 重建路径：读取relations-cache → 收集三类向量条目 → 分批向量化（200条/批）→ memoryId回写 → 统计报告。
...
participant RV as "rebuild-vector.ts"
...
Note over RV : 重建流程
RV->>RV : collectContentEntries()
RV->>RV : collectPathRelationEntries()
RV->>RV : collectTagEntries()
RV->>VC : vectorBulkStore(entries, batch=200)
VC->>Eng : bulk upsert
Eng->>W : batch insert
W->>Z : bulk upsert
Z-->>W : results
W-->>Eng : WriteResult
Eng-->>VC : BulkWriteResult
VC-->>RV : results with memoryIds
RV->>RV : updateMemoryIds()
RV-->>App : RebuildVectorResult
```
...
- [rebuild-vector.ts:428-516](file://src/lib/rebuild-vector.ts#L428-L516)
...
**新增功能**：
- 200条/批的分批处理：VECTORIZE_BATCH_SIZE常量定义批大小，避免一次性提交大量数据
- 实时进度报告：每批完成后调用logProgress输出进度，用户可感知处理进度
- 确定性文档ID：基于text+scope+tag生成，确保幂等性和可重现性
- 全面统计收集：返回ok Map和errors数组，精确记录成功和失败条目

章节来源
- [batch-vectorize.ts:44-95](file://src/lib/batch-vectorize.ts#L44-L95)
- [batch-vectorize.ts:129-183](file://src/lib/batch-vectorize.ts#L129-L183)

### 向量重建（rebuild-vector.ts）
- 三类向量重建：ki-search（内容向量）、ki-relation（关系向量）、ki-path（路径向量）
- 局部重建支持：--group过滤子树、--tags打标合并，不清空其他向量
- 内存ID回写：成功后将docId回写到relations-cache.json的rel.memoryId
- 标签合并去重：--tags与现有rel.tags合并，只增不减，跨命令累积

**增强功能**：
- 分批处理：200条/批，与import链路保持一致，批间输出进度
- 进度报告：全量重建时显示删除进度，向量化过程显示批次进度
- 统计收集：content/relation/path/tag/succeeded/failed/updatedMemoryId/taggedRelations/mergedTags
- 错误处理：失败条目精确定位，包含type/path/error详细信息

章节来源
- [rebuild-vector.ts:102-159](file://src/lib/rebuild-vector.ts#L102-L159)
- [rebuild-vector.ts:166-189](file://src/lib/rebuild-vector.ts#L166-L189)
- [rebuild-vector.ts:196-239](file://src/lib/rebuild-vector.ts#L196-L239)
- [rebuild-vector.ts:248-281](file://src/lib/rebuild-vector.ts#L248-L281)
- [rebuild-vector.ts:291-309](file://src/lib/rebuild-vector.ts#L291-L309)
- [rebuild-vector.ts:329-520](file://src/lib/rebuild-vector.ts#L329-L520)
...
- 耦合与内聚
  ...
  - rebuild-vector 与 vector-client 松耦合，通过依赖注入支持测试mock。
- 外部依赖
...
- [siliconflow.ts:43-102](file://src/zvec-engine/embedding/siliconflow.ts#L43-L102)
- [config.ts（lib）:120-140](file://src/lib/config.ts#L120-L140)
- [rebuild-vector.ts:27-33](file://src/lib/rebuild-vector.ts#L27-L33)
- [batch-vectorize.ts:17-19](file://src/lib/batch-vectorize.ts#L17-L19)
- [restore.ts:27](file://src/restore.ts#L27)
...
- 写入阶段
  ...
  - 重建优化：rebuildScopeVectors 同样采用200条/批，支持进度报告和统计收集。
- 连接与并发
...
**新增优化**：
- 分批处理：200条/批平衡了内存使用和进度反馈频率
- 进度报告：批间logProgress调用提供实时处理进度
- 统计收集：全面的stats对象便于监控和分析重建效果
- 内存ID回写：避免悬空引用，提升数据一致性
...
- [config.ts（lib）:220-290](file://src/lib/config.ts#L220-L290)
- [batch-vectorize.ts:129-183](file://src/lib/batch-vectorize.ts#L129-L183)
- [rebuild-vector.ts:468-516](file://src/lib/rebuild-vector.ts#L468-L516)
...
- 重建相关错误
  - 现象：rebuildScopeVectors返回errors数组，包含type/path/error详细信息。
  - 处置：根据错误类型定位问题，如'scope'表示scope目录不存在，'cache'表示relations-cache.json缺失，'cleanup'表示清空旧向量失败。

**新增故障排查**：
- 分批处理问题：检查VECTORIZE_BATCH_SIZE设置，确认批间进度是否正常输出
- 进度报告异常：验证logProgress调用是否正确，检查totalBatches计算逻辑
- 统计信息不完整：确认aggResults聚合逻辑，检查index偏移校正是否正确
- memoryId回写失败：验证updateMemoryIds匹配键格式，检查relations-cache.json写入权限
...
- [rebuild-vector.ts:357-422](file://src/lib/rebuild-vector.ts#L357-L422)
- [rebuild-vector.ts:502-516](file://src/lib/rebuild-vector.ts#L502-L516)
...
**最新增强**：批量向量化和向量重建功能的增强，特别是200条/批的分批处理、实时进度报告、确定性文档ID生成和全面统计收集，显著提升了大规模知识库处理的性能和可观测性。这些改进使得系统能够更好地处理大型知识图谱的重建和维护任务，为用户提供更可靠的向量检索服务。