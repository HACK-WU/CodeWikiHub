...
**所做更改**
- 新增多scope聚合查询功能说明
- 更新搜索执行流程图以反映多scope支持
- 增强架构总览部分以包含多scope处理逻辑
- 添加多scope查询的性能优化和实现细节
- 更新故障排查指南以涵盖多scope相关场景
...
## 简介
本文面向 knowledge-indexer 的"搜索执行流程"，完整描述从用户查询到结果返回的处理链路，包括查询解析、标签过滤、混合检索、结果排序、原文定位与上下文增强。**最新增强**：支持多scope聚合查询，当指定多个scope时执行单次embedding查询与OR过滤，返回统一排序的结果集，每个命中都标注其来源scope。重点解释双路径查询机制（索引直查与语义检索协同）、RRF 融合排序、结果去重与上下文增强机制，并提供搜索参数配置、性能优化技巧与常见查询模式的实现指引。
...
## 核心组件
- 搜索入口 executeSearch：解析 scope、校验向量服务可用性、**支持多scope聚合查询**、按 tag 分查或单查、召回后附加 group/relation、可选召回原文、多 tag 去重、同文件多 chunk 去重
- 向量客户端 vectorSearch：构建 scope+tag 过滤，**支持多scope OR 过滤**，调用引擎 hybridSearch，阈值过滤，标准化返回
...
## 架构总览
下图展示一次搜索从 CLI 到引擎再到结果返回的端到端流程，包含**多scope聚合查询**、双路径混合检索与 RRF 融合。
...
U->>CLI : 输入查询/参数(可含多scope)
CLI->>CLI : parseScopes() 解析多scope
CLI->>VC : vectorSearch(scopes[], query, limit, threshold, tags)
VC->>VC : buildScopeTagFilter() 构建OR过滤条件
VC->>ENG : hybridSearch(queryText, fts, topk, filter)
ENG->>RT : routeSearch(含queryText/fts)
RT-->>ENG : multiQuery(语义路 + FTS路)
ENG-->>VC : 原始命中列表(带scope字段)
VC-->>CLI : 标准化结果 + 阈值过滤
CLI->>RM : getRelationMap(scope) 为每个scope建立映射
RM-->>CLI : memoryId → {group, relation}
CLI->>CLI : 多tag去重 / 同文件chunk去重
CLI-->>U : 最终搜索结果(每条命中标注来源scope)
```
...
### 搜索入口：executeSearch
- 作用：统一编排搜索生命周期，处理 **多scope解析**、向量可用性检测、标签策略、原文召回、去重与排序
- **新增多scope支持**：
  - 支持逗号分隔的多scope输入（如 `scope: 'team-a,team-b'`）
  - 在strict模式下，未注册的scope会被跳过并记录原因，不影响其他有效scope的检索
  - 所有有效scope合并为单一查询，执行一次embedding和一次向量检索
  - 返回结果中每个命中都标注其来源scope字段
- 关键逻辑
...
```mermaid
flowchart TD
S["开始"] --> P["parseScopes() 解析多scope"]
P --> V{"单scope还是多scope?"}
V -- 单scope --> Q1["单次vectorSearch(tags=传入)"]
V -- 多scope --> Q2["构建OR过滤条件<br/>单次vectorSearch(scopes[])"]
Q1 --> M["构建relation-map"]
Q2 --> M
M --> O{"includeOriginal?"}
...
### 向量客户端：vectorSearch
- 作用：构造 **多scope OR 过滤条件** + tag 过滤，调用引擎 hybridSearch，并将底层 Hit 标准化为 VectorSearchResult
- **多scope支持关键点**：
  - buildScopeTagFilter：强制 scope 相等，**多scope时构建OR条件**，tags 非空时以 OR 组合，忽略大小写
  - 阈值过滤：score < threshold 的结果被丢弃
  - 字段映射：memoryId、content、score、tag、**group、scope**（来源scope）

```mermaid
flowchart TD
A["vectorSearch(params)"] --> B["buildScopeTagFilter(scopes[], tags)"]
B --> C["engine.hybridSearch(queryText, fts, topk, filter)"]
C --> D["标准化Hit为VectorSearchResult<br/>包含scope字段"]
D --> E{"score >= threshold?"}
E -- 否 --> F["丢弃"]
E -- 是 --> G["返回结果"]
```
...
- **多scope去重增强**：去重key包含scope字段，确保不同scope的同名文档不会被错误去重
...
- **核心特性**：
  - 支持逗号分隔的多scope输入（如 `scope: 'team-a,team-b'`）
  - 执行单次embedding查询和单次向量检索，而非对每个scope分别查询
  - 构建OR过滤条件：`(scope == 'team-a' OR scope == 'team-b') AND (其他过滤条件)`
  - 返回结果中每个命中都标注其来源scope字段
  - strict模式下未注册scope会被跳过并记录原因，不影响其他有效scope
- **实现细节**：
  - parseScopes()：解析逗号分隔的scope字符串，去空格、去重、保序
  - buildScopeTagFilter()：为多scope构建OR过滤条件
  - 响应结构：多scope查询返回scopes[]数组和skipped[]信息
  - 权限控制：HTTP鉴权闸门支持多scope逐段校验

**章节来源**
- [src/search.ts:93-127](file://src/search.ts#L93-L127)
- [src/lib/vector-client.ts:658-667](file://src/lib/vector-client.ts#L658-L667)
...
- **多scope聚合优化**：
  - 单次embedding：多scope查询仅执行一次embedding，避免N次重复计算
  - 单次向量检索：通过OR过滤条件一次性跨scope检索，减少网络往返
  - 内存效率：使用Map结构高效管理多scope的关系映射
...
- 建议
  ...
  - **多scope查询适合跨团队/项目的统一检索场景，注意合理控制scope数量**
...
- **多scope相关故障**：
  - strict模式下未注册scope：会被跳过并在skipped字段中记录原因，不影响其他有效scope
  - 全部scope未注册：返回ok:false并提示"无可检索的scope"
  - 权限越权：HTTP鉴权会拒绝包含未授权scope的请求
...
## 结论
knowledge-indexer 的搜索执行流程通过"语义检索 + FTS 关键词"的双路径混合检索与 RRF 融合排序，实现了高召回与高相关性的平衡。**最新增强的多scope聚合查询功能**进一步提升了跨团队、跨项目的统一检索能力，通过单次embedding和OR过滤机制显著优化了性能。配合标签过滤、结果去重、原文定位与上下文增强，既保证了准确性，又提升了用户体验。通过合理的参数配置与性能优化策略，可在不同规模的知识库中稳定高效地提供服务。

[本节为总结，无需具体文件引用]

## 附录：参数与示例
- 搜索参数
  - scope：项目隔离标识（default/strict 模式），**支持逗号分隔多scope聚合查询**
  - query：自然语言查询文本
  - limit：返回条数上限
...
- **多scope查询示例**：
  ```bash
  # 单scope查询
  ki search "告警配置" -s monitor
  
  # 多scope聚合查询（逗号分隔）
  ki search "告警配置" -s monitor,alerting,logging
  
  # 多scope查询响应包含scopes[]和skipped[]信息
  ```
- 常见查询模式
  ...
  - **多scope聚合**：跨团队/项目统一检索，单次查询返回统一排序结果
- 性能调优建议
  - 设置合适的 threshold 过滤低分结果
  - 使用 tags 缩小范围，减少扫描量
  - 在大库上优先使用 FTS 关键词预筛，再叠加语义检索
  - 按需开启 original，避免频繁文件读取
  - **多scope查询时合理控制scope数量，避免过多scope导致过滤条件复杂化**
