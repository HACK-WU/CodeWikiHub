...
## 更新摘要
**变更内容**
- 新增parseScopes工具函数的详细说明，包括逗号分隔解析、空白字符修剪、去重保序功能
- 增强ScopeError异常处理机制的说明，涵盖空段处理和路径穿越防护
- 更新多Scope支持的相关章节，体现新的解析逻辑
- 补充parseScopes在搜索流程中的应用示例
...
- **新增** parseScopes工具函数：专门处理逗号分隔的scope参数解析，支持空白字符修剪、去重保持顺序、输入验证。
...
CFG-->>U : KiConfig
U->>SCOPE : validateScope/getKbDir/parseScopes
SCOPE-->>U : 路径/校验结果
...
### **新增** parseScopes工具函数详解
**新增** parseScopes函数是Scope隔离机制中的重要工具函数，专门处理逗号分隔的多scope参数解析：

#### 核心功能特性
- **空白字符修剪**：自动去除每个scope前后的空格和空白字符
- **去重保持顺序**：使用Set数据结构确保scope唯一性，同时保持原始输入顺序
- **空段过滤**：自动忽略空字符串和纯空白段
- **输入验证**：对每个解析后的scope段进行validateScope校验
- **安全快速失败**：遇到非法字符或路径穿越尝试立即抛出ScopeError

#### 解析流程
```mermaid
flowchart TD
Input["输入: 'team-a, team-b ,team-a'"] --> Split["split(',') → ['team-a', ' team-b ', 'team-a']"]
Split --> Trim["trim() → ['team-a', 'team-b', 'team-a']"]
Trim --> Filter["filter(Boolean) → 移除空段"]
Filter --> Validate["逐个validateScope校验"]
Validate --> Dedup["Set去重保序 → ['team-a', 'team-b']"]
Dedup --> Output["输出: ['team-a', 'team-b']"]
```
...
#### 异常处理机制
- **EMPTY_SCOPE异常**：当输入包含有效字符但解析后无有效段时抛出
- **INVALID_SCOPE异常**：当任何scope段包含非法字符时抛出，包含具体非法字符信息
- **路径穿越防护**：拒绝包含`..`等路径遍历字符的scope名称

#### 使用示例
```typescript
// 基础用法
parseScopes('team-a, team-b ,team-a') // → ['team-a', 'team-b']
parseScopes('solo') // → ['solo']
parseScopes('team-a,,team-b,') // → ['team-a', 'team-b']
...
// 异常情况
parseScopes(', ,') // → 抛出 EMPTY_SCOPE 异常
parseScopes('team-a,../etc') // → 抛出 INVALID_SCOPE 异常
```

**章节来源**
- [src/lib/scope.ts:53-69](file://src/lib/scope.ts#L53-L69)
- [test/search-multiscope.test.ts:52-76](file://test/search-multiscope.test.ts#L52-L76)
...
### 搜索中的Scope行为
**更新** 搜索流程现已集成parseScopes函数以支持多scope检索：

- 入口executeSearch先通过parseScopes解析多scope参数，再进行resolveScope护栏
- 支持逗号分隔的多scope查询：`ki search --scope "team-a,team-b" --query "..."`
- 单scope和多scope的不同处理策略：
  - 单scope：保持原有行为，strict模式下未注册直接失败
  - 多scope：strict模式下未注册scope被跳过并记录原因，继续处理其他有效scope
...
**图表来源**
- [src/search.ts:90-140](file://src/search.ts#L90-L140)
...
- **新增** parseScopes函数被search.ts等模块调用以支持多scope检索。
...
- 搜索阶段：
  ...
  - **新增** parseScopes使用Set进行去重，时间复杂度O(n)，空间复杂度O(n)。
- 导入阶段：
...
- **新增** parseScopes解析异常：
  - 现象：报错"解析为空：逗号分隔列表无有效段"。
  - 原因：输入包含有效字符但解析后无有效段（如纯分隔符）。
  - 处理：检查输入格式，确保至少有一个有效的scope名称。
- **新增** 路径穿越防护：
  - 现象：报错"不合法：仅允许字母、数字、连字符(-)、下划线(_)，禁止路径遍历字符"。
  - 原因：parseScopes检测到包含`..`等路径遍历字符的scope。
  - 处理：修正scope名称，移除路径遍历字符。
- 向量服务不可用：
...
## 结论
Scope隔离机制通过配置驱动的模式与路径策略，实现了项目级数据隔离、命名空间管理与严格/灵活的访问控制。**新增的parseScopes工具函数**进一步增强了多scope检索的支持，提供了健壮的参数解析、输入验证和安全防护机制。配合CLI的生命周期管理、搜索与导入的流程集成、以及备份恢复能力，形成了完整的多项目知识索引体系。在生产环境中建议采用strict模式与白名单注册，合理划分kbDir与dataDir，并建立完善的备份与监控策略。
...
- **新增** 多scope搜索：
  - ki search --scope "team-a,team-b" --query "..." [--limit 10] [--threshold 0.0] [--tags ...] [--original]
- 搜索：
