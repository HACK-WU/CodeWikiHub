...
**关键发现**：① 后端 `ok === false` 业务错误不抛异常，是 api 层与页面的隐式约定（C1 已记录）；② MarkdownPreview 的 XSS 防线**不是一道而是三道**（HTML 丢弃+`<img>` 净化重建 / URL scheme 白名单 / mermaid strict）。2026-09-08 前存在两处真实缺口——`renderer.link` 未覆盖（`javascript:`/`data:` 链接点击即执行）与 `mermaid securityLevel:'loose'`（跳过 DOMPurify 且启用 click JS 回调），**均已修复并浏览器实测**，详见 C1；教训：「丢了 HTML 所以安全」这个直觉只覆盖标签注入一条路径，URL scheme 与图表引擎是两条独立路径；③ Browse/Search 两页 tag 来源不同（KB 层 vs 向量层）是有意设计，易被误判为 bug。
