...
- [src/import.ts](file:///root/knowledge-indexer/src/import.ts)
...
```
CLI 层           src/import.ts（扁平命令，无子命令；2026-09-07 由 scan-kb 壳扁平化）
...
```mermaid
graph TD
    A[src/import.ts CLI] --> B[lib/import.ts handleDirectImport]
...
| 依赖方 | 被依赖方 | 说明 |
|--------|----------|------|
| `src/import.ts`（CLI） | `lib/import.ts` / `backup.ts`（autoBackup）/ `vector-client.ts`（closeEngine）/ `clean.ts` | CLI 入口 |
| `mcp-http-api.ts` | `lib/import.ts`（handleDirectImport） | HTTP 导入异步 job |
| `lib/import.ts`（编排） | `interrupt.ts`（锁/标记）/ `clean.ts` / `chunker.ts` / `batch-vectorize.ts` / `path-vectorize.ts` / `ai-results.ts`（deriveGroupPath）/ `scope.ts` / `store.ts` / `config.ts` / `progress.ts` | 直导编排 |
...
章节来源：[src/import.ts](file:///root/knowledge-indexer/src/import.ts)（commander 扁平 program 的 action）、[src/lib/mcp-http-api.ts](file:///root/knowledge-indexer/src/lib/mcp-http-api.ts)（`handleImportRun`）、[src/lib/vector-client.ts](file:///root/knowledge-indexer/src/lib/vector-client.ts)（`getEngine`）
