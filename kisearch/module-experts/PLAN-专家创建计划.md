...
> ⚠️ **历史快照（2026-08-06），不随代码演进维护**。本文件是专家团创建当时的计划记录，其中的命令名、模块文件、测试文件与用例数均为**当时状态**，多处已随后续迭代失效（如 `import-kb.ts` / `incremental` / `ai-results` 输入契约已删、CLI 命令数已变、`ki scan-kb import` 已于 2026-09-07 扁平化为 `ki import`）。故本文件**刻意保留当时拼写**、不跟随重命名；**查阅现行事实请以 `PROJECT.md` 与各专家 `C0/C1` 为准，勿据本文件调用命令。**
